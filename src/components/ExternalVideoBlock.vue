<template>
    <div class="es-external-video-block">
        <div
            class="es-external-video-container"
            :class="{ 'is-hidden': !content.url }"
        >
...
export default {
    ...
    methods: {
        async handleGenerateVideoPoster() {
            const saveButtonNode = document.querySelector(
                '.k-page-view-header [aria-label="Save"]'
            );
            if (saveButtonNode) {
            ...
            canvas.toBlob(
                async (blob) => {
                    try {
                        ...
                        if (!response.ok)
                            throw new Error(
                                "(External Video) Upload failed - Response Status: " +
                                    response.status
                            );
                        ...
                        console.error(
                            "(External Video) Upload failed - Error:",
                            err
                        );
                    } finally {
                        canvas.width = 0;
